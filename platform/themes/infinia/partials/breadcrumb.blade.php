...
@if($breadcrumbEnabled)
    <section class="section-page-header py-10 fix position-relative">
                    ...
                    @foreach (Theme::breadcrumb()->getCrumbs() as $crumb)
                        @if($loop->last)
                            <li class="d-flex align-items-center">
                                ...
                                @endif
                                <p class="text-primary mb-0">{{ $crumb['label'] }}</p>
                            </li>
                        @else
                            <li>
                                <a href="{{ $crumb['url'] }}">
        ...
        @if($backgroundImage = theme_option('breadcrumb_background_image'))
            {{ RvMedia::image($backgroundImage, __('Background image'), attributes: ['class' => 'position-absolute bottom-0 start-0 end-0 top-0 z-0']) }}
        @endif
