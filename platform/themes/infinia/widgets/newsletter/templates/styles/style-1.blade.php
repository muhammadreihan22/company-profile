                ...
                <div class="px-lg-5 text-lg-start text-center">
                    @if($config['subtitle'])
                        <div class="d-flex align-items-center justify-content-center bg-primary-soft border border-2 border-white d-inline-flex rounded-pill px-4 py-2" data-aos="zoom-in" data-aos-delay="100">
                            ...
                            <span class="tag-spacing fs-7 fw-bold text-linear-2 ms-2">{{ $config['subtitle'] }}</span>
                        </div>
                    @endif
                    @if($config['title'])
                        <h4 class="mt-3 mb-3" data-aos="fade-zoom-in" data-aos-delay="100">{{ $config['title'] }}</h4>
                    @endif
                    @if($config['description'])
                        <span class="fs-6 fw-medium" data-aos="fade-zoom-in" data-aos-delay="200">{!! BaseHelper::clean(nl2br($config['description'])) !!}</span>
                    @endif
